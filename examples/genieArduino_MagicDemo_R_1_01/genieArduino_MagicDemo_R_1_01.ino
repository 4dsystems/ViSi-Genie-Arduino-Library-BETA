  ...
  if (millis() >= waitPeriod) {
    if (genie.online(0)) { // check if the display is online (connected) and if on Form0
      ...
      if (alternateFlag == SEND_BYTES) {
        ...
        Serial.println();
        genie.WriteMagicBytes(0, magicBytes, messageLength, 1); // send the magic bytes
        ...
        Serial.println();
        genie.WriteMagicDBytes(0, magicDBytes, messageLength, 1); // send the magic double bytes
...
uint8_t receivedMagicBytes[255];
void myGenieMagicBHandler(uint8_t index, uint8_t length) {  
  byte receivedChkSum, generatedChkSum;
  generatedChkSum = GENIEM_REPORT_BYTES ^ index ^ length;
  
  for (int i = 0; i < length; i++) {
    receivedMagicBytes[i] = genie.GetNextByte(); // get a magic byte
    generatedChkSum ^= receivedMagicBytes[i]; // compute for the checksum
  }
  receivedChkSum = genie.GetNextByte(); // get the checksum

  if(generatedChkSum != receivedChkSum){
  ...
  if (index == 0) // If the data is coming from MagicObject0
  {
    ...
    for (int i = 0; i < length; i++ ) {
      Serial.print(receivedMagicBytes[i], HEX);
      Serial.print(" ");
    }
    Serial.println();
    Serial.println();
...
uint16_t receivedMagicDoubleBytes[255];
void myGenieMagicDBHandler(uint8_t index, uint8_t length) {

  byte receivedChkSum, generatedChkSum;
  generatedChkSum = GENIEM_REPORT_DBYTES ^ index ^ length;
  
  for (int i = 0; i < length; i++) {
    receivedMagicDoubleBytes[i] = genie.GetNextDoubleByte();
    generatedChkSum ^= (byte)(receivedMagicDoubleBytes[i] >> 8);
    generatedChkSum ^= (byte)(receivedMagicDoubleBytes[i] & 0xFF);
  }
  receivedChkSum = genie.GetNextByte();
  ...
  if (index == 0) // If the data is coming from MagicObject0
  {
    ...
    for (int i = 0; i < length; i++ ) {      
      Serial.print((char)(receivedMagicDoubleBytes[i] >> 8));
      Serial.print(" ");
      Serial.print((char)(receivedMagicDoubleBytes[i] & 0xFF));
      Serial.print(" ");
    }
    Serial.println();
    Serial.println();
